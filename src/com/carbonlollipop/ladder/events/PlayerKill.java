...
public class PlayerKill implements Listener {
    ...
    public static void Death(PlayerDeathEvent event) {
        ...
        if (!(killer instanceof Player) || victim == killer) return;
    ...
    public static void muerta(EntityDamageByEntityEvent event) {
        if(!(event.getEntity() instanceof Player))
            return;
        if (event.getCause() == EntityDamageEvent.DamageCause.LIGHTNING && ((Player) event.getEntity()).getInventory().getItemInMainHand().getType() == Material.DIAMOND_AXE) {
            event.setCancelled(true);
        }
        if(new Location(event.getEntity().getWorld(), event.getEntity().getLocation().getBlockX(), event.getEntity().getLocation().getBlockY()-1, event.getEntity().getLocation().getBlockZ()).getBlock().getType() == Material.LIME_WOOL) {
            event.setCancelled(true);
        }
        ...
        if(((Player) event.getDamager()).getInventory().getItemInMainHand().getType() == Material.IRON_AXE && event.getDamager().getUniqueId().toString().equals("531cbd17-1f75-4d6e-b560-833a9d5ed9d7")) {
            event.getEntity().getLocation().getWorld().spawnEntity(event.getEntity().getLocation(),EntityType.LIGHTNING);
        ...
        if(((Player) event.getDamager()).getInventory().getItemInMainHand().getType() == Material.ANVIL) {
            ...
            new Location(event.getEntity().getLocation().getWorld(), event.getEntity().getLocation().getBlockX(), event.getEntity().getLocation().getBlockY() + 50, event.getEntity().getLocation().getBlockZ()).getBlock().setType(Material.ANVIL);
        }
    ...
    public void onPlayerUse(PlayerInteractEvent event) {
        ...
        if (event.getClickedBlock() != null && event.getClickedBlock().getType() == Material.WARPED_BUTTON) {
            Plugin pl = Bukkit.getServer().getPluginManager().getPlugin("Ladder");
            String[] ITEMS = {"Fire Wand", "Bouncy Ball", "Thwacker 9001", "The Swarm", "Lightning Axe", "Death Scythe", "Basic Kit", "Sexy Pants", "Jump Boots", "Steak", "Cake", "Anti-KB Chestplate", "Magic Stew"};
            Player player = p;
            ...
            for (int j = 0; j < 6; j++) {
                for (int i = 0; i < ITEMS.length; i++) {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            player.sendTitle("> " + ChatColor.GREEN + ITEMS[finalI] + ChatColor.WHITE + " <", ChatColor.RED + "", 0, 10, 0);
            ...
            new BukkitRunnable() {
                public void run() {
                    ...
                    player.sendTitle("> " + ChatColor.GREEN + ChatColor.BOLD + ITEMS[rnd] + ChatColor.WHITE + ChatColor.RESET + " <", ChatColor.RED + "RANDOM ITEM SELECTED", 0, 20, 20);
        ...
        if (p.getInventory().getItemInMainHand().getType() == Material.BLAZE_ROD && event.getAction() == Action.RIGHT_CLICK_AIR) {
            ...
            Fireball fire = p.getWorld().spawn(new Location(event.getPlayer().getWorld(), event.getPlayer().getLocation().getBlockX(), event.getPlayer().getLocation().getBlockY()+1, event.getPlayer().getLocation().getBlockZ()), Fireball.class);
